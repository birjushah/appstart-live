...
class ModuleImageGallery_Form_ModuleImageGallery extends Standard_Form{
	public function init(){
		...
		//  Image Gallery Id
		$image_gallery_id = $this->createElement ( "hidden", "module_image_gallery_id", array (
				'value' => '',
				'filters' => array (
						'StringTrim'
				)
		...
		$image_gallery_id->setAttribs(array(
				...
				'name' => 'data[1][module_image_gallery_id]'
		));
		$this->addElement ( $image_gallery_id);
		
		//  Image Gallery Detail Id
		$image_gallery_detail_id = $this->createElement ( "hidden", "module_image_gallery_detail_id", array (
				'value' => '',
				'filters' => array (
						'StringTrim'
				)
		...
		$image_gallery_detail_id->setAttribs(array(
				...
				'name' => 'data[1][module_image_gallery_detail_id]'
		));
		...
		$image_gallery_category_id->setAttribs(array(
				...
				'name' => 'data[1][image_gallery_category_id]'
		));
		$this->addElement ( $image_gallery_category_id );
		
		//Select Category
		$categories = $this->_getCategories();
		$selectcategory = $this->createElement('select','module_image_gallery_category_id',array(
		...
		$selectcategory->setAttribs(array(
				...
				'name' => 'data[1][module_image_gallery_category_id]'
		));
		...
		$image->setLabel(false)
			 ->setDestination(Standard_Functions::getResourcePath(). "module-image-gallery/images/")
