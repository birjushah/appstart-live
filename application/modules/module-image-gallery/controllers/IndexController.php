...
class ModuleImageGallery_IndexController extends Zend_Controller_Action {
	...
	public function init() {
		...
		$customer_id = Standard_Functions::getCurrentUser()->customer_id;
		$DBExpr = new Zend_Db_Expr("COUNT(module_image_gallery_id)");
		$select = $mapper->getDbTable()->select(false)
		->setIntegrityCheck(false)
		->from('module_image_gallery',array('count'=>$DBExpr ))
	...
	public function editAction() {
		...
		if ($request->getParam ( "id", "" ) != "" && $request->getParam ( "lang", "" ) != "") {
			$mapper = new ModuleImageGallery_Model_Mapper_ModuleImageGallery ();
			$module_image_gallery_id = $request->getParam ( "id", "" );
			...
			$data = $mapper->find ( $module_image_gallery_id )->toArray ();
			$form->populate ( $data );
			$datadetails = array ();
			$detailsMapper = new ModuleImageGallery_Model_Mapper_ModuleImageGalleryDetail ();
			if ($detailsMapper->countAll ( "module_image_gallery_id = " . $module_image_gallery_id . " AND language_id = " . $language_id ) > 0) {
				// Record For Language Found
				$dataDetails = $detailsMapper->getDbTable ()->fetchAll ( "module_image_gallery_id = " . $module_image_gallery_id . " AND language_id = " . $language_id )->toArray ();
				$keywords = $dataDetails [0] ['keywords'];
				$keywords = explode ( ",", $keywords );
			} else {
				// Record For Language Not Found
				$dataDetails = $detailsMapper->getDbTable ()->fetchAll ( "module_image_gallery_id = " . $module_image_gallery_id . " AND language_id = " . $default_lang_id )->toArray ();
				$dataDetails [0] ["module_image_gallery_detail_id"] = "";
			...
			if (isset ( $dataDetails [0] ) && is_array ( $dataDetails [0] )) {
				...
				$image_uri = "resource/module-image-gallery/thumb/" . $customer_id . "/";
				$ext_image_path = array_pop ( explode ( ".", $image_path ) );
	...
	public function deleteAction() {
		...
		if (($imageId = $request->getParam ( "id", "" )) != "") {
    		...
    		{
	    		$image = new ModuleImageGallery_Model_ModuleImageGallery();
	    		...
	    		if($image) {
	    			...
	    			try {
	    				$detailsMapper = new ModuleImageGallery_Model_Mapper_ModuleImageGalleryDetail();
	    				$details = $detailsMapper->fetchAll("module_image_gallery_id=".$image->getModuleImageGalleryId());
	    				if(is_array($details)) {
	...
	public function gridAction() {
		...
		$select = $mapper->getDbTable ()->select ( false )->setIntegrityCheck ( false )->from ( array (
				"mig" => "module_image_gallery" 
		), array (
				"mig.module_image_gallery_id" => "module_image_gallery_id",
				"mig.module_image_gallery_category_id" => "mig.module_image_gallery_category_id",
				"mig.status" => "status",
				"mig.order" => "order" 
		) )->joinLeft ( array (
		...
		), 		"migcd.module_image_gallery_category_id = mig.module_image_gallery_category_id AND migcd.language_id = " . $active_lang_id, array (
				"migcd.title" => "title", 
		) )->joinLeft ( array (
				"migc" => "module_image_gallery_category"
		),		"migc.module_image_gallery_category_id = mig.module_image_gallery_category_id", array(
				"migc.status" => "status"
		));
		if ($category_id != "") {
			$select = $select->where ( "migc.status=1 AND mig.module_image_gallery_category_id =".$category_id." AND mig.customer_id=" . Standard_Functions::getCurrentUser ()->customer_id );
		} else {
			$select = $select->where ( "migc.status=1 AND mig.customer_id=" . Standard_Functions::getCurrentUser ()->customer_id );
		}
		/**
		* Special patch for title
		*/
		/*$migdTitle = $request->getParam("search_migd","");		
		if($migdTitle["title"]==null){
			$select->orWhere("migd.title is null");
		}*/
		...
		foreach ( $rows as $rowId => $row ) {
			...
			//print_r($row[6]);
			//die;
			if ($row [6] ["migd.module_image_gallery_detail_id"] == "") {
				$mapper = new ModuleImageGallery_Model_Mapper_ModuleImageGalleryDetail ();
				$details = $mapper->fetchAll ( "module_image_gallery_id=" . $row [6] ["mig.module_image_gallery_id"] . " AND language_id=" . $default_lang_id );
				if (is_array ( $details )) {
					$details = $details [0];
			...
			if ($languages) {
				foreach ( $languages as $lang ) {
					$editUrl = $this->view->url ( array (
							...
							"controller" => "index",
							"action" => "edit",
							"id" => $row [6] ["mig.module_image_gallery_id"],
			...
			$response ['aaData'] [$rowId] [0] = "<input type='checkbox' name='image_id' class='image_id' value='".$row [6] ["mig.module_image_gallery_id"]."' />";
	...
	public function reorderAction() {
		...
		if ($this->_request->isPost ()) {
			...
			try {
				...
				$mapper->getDbTable ()->getAdapter ()->commit ();
				if ($model && $model->getModuleImageGalleryId () != "") {
					$response = array (
							"success" => true 
					);
				}
		...
		$select = $mapper->getDbTable ()->select ( false )->setIntegrityCheck ( false )->from ( array (
				...
				"mig.module_image_gallery_id" => "module_image_gallery_id",
				"mig.module_image_gallery_category_id" => "module_image_gallery_category_id",
				"mig.status" => "status",
				"mig.order" => "order" 
		) )->joinLeft ( array (
				"migd" => "module_image_gallery_detail" 
				...
				"migd.title" => "title",
				"migd.image_path" => "image_path" 
		) )->where ( "mig.module_image_gallery_category_id ='" . $category_id . "'AND mig.customer_id=" . $customer_id )->order ( "mig.order" );
