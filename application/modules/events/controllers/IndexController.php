...
class Events_IndexController extends Zend_Controller_Action
{
	...
	public function indexAction()
	{
		...
		$customer_id = Standard_Functions::getCurrentUser()->customer_id;
		$DBExpr = new Zend_Db_Expr("COUNT(module_events_category_id)");
		$select = $mapper->getDbTable()->select(false)
		->setIntegrityCheck(false)
		->from('module_events_category',array('count'=>$DBExpr ))
		->where("customer_id =".$customer_id);
		$stack = $mapper->getDbTable()->fetchRow($select)->toArray();
		$this->view->totalcategories = $stack['count'];
		$form = new Events_Form_EventsBulkEdit();
		$this->view->form = $form;
	}
	...
	public function editAction()
	{
		...
		if ($request->getParam ( "id", "" ) != "" && $request->getParam ( "lang", "" ) != "") {
			...
			if(isset($dataDetails[0]) && is_array($dataDetails[0])) {
				...
				//die();
				if($dataDetails[0]["start_date_time"] != ""){
					$start_date = DateTime::createFromFormat ( "Y-m-d H:i:s", $dataDetails[0]["start_date_time"] );
					if($start_date){
						$dataDetails[0]["start_date_time"] = $start_date->format ( "d/m/Y H:i" );
					}
				}
				if($dataDetails[0]["end_date_time"] != ""){
					$end_date = DateTime::createFromFormat ( "Y-m-d H:i:s", $dataDetails[0]["end_date_time"] );
					if($end_date){
						$dataDetails[0]["end_date_time"] = $end_date->format ( "d/m/Y H:i" ) ;
					}
				}
		...
		$customer_id = Standard_Functions::getCurrentUser ()->customer_id;
		$this->view->categoryTree = $this->_getCategoryTree ( $customer_id, $lang_id);
		$parent_id = $data["module_events_category_id"];
		$this->view->orignalParent = $parent_id;
		$detailMapper = new Events_Model_Mapper_ModuleEventsCategoryDetail();
		if($parent_id != 0){
			$details = $detailMapper->getDbTable()->fetchAll("language_id = ".$lang_id." AND module_events_category_id =" .$parent_id)->toArray();
		...
		if ($this->_request->isPost ()) {
			...
			if ($form->isValid ( $this->_request->getParams () )) {
				...
				try {
					...
					if($model && $model->getModuleEventsId()!="") {
						$response = array (
								"success" => $model->getModuleEventsId()
						);
					}
				} catch (Exception $ex) {
					$response = array (
	...
	public function bulkEditAction() {
		...
		if ($this->_request->isPost ()) {
			try {
				$module_events_id = $request->getParam("module_events_id",false);
				$language_id = $request->getParam("language_id",false);
				$start_date_time = $request->getParam("start_date_time","");
				$end_date_time = $request->getParam("end_date_time","");

				if($start_date_time != ""){
					$start_date = DateTime::createFromFormat ( "d/m/Y H:i", $start_date_time);
					if($start_date){
						$start_date_time = $start_date->format ( "Y-m-d H:i:s" );
					}
				}
				if($end_date_time != ""){
					$end_date = DateTime::createFromFormat ( "d/m/Y H:i", $end_date_time );
					if($end_date){
				...
				$eventdetailMapper = new Events_Model_Mapper_ModuleEventsDetail();
				$eventdetailMapper->getDbTable()->getAdapter()->beginTransaction();
				if(is_array($module_events_id) && is_array($language_id)) {
					foreach($module_events_id as $events_id) {
						foreach($language_id as $lang_id) {
							$eventsDeatil = $eventdetailMapper->fetchAll("module_events_id ='".$events_id."' AND language_id =".$lang_id);
							if(is_array($eventsDeatil) && isset($eventsDeatil[0])) {
								$eventsDeatil = $eventsDeatil[0];
								$eventsDeatil->setStartDateTime ( $start_date_time );
								$eventsDeatil->setEndDateTime ( $end_date_time );
								$eventsDeatil->setLastUpdatedBy ( $user_id );
								$eventsDeatil->setLastUpdatedAt ( $date_time );
								$eventsDeatil = $eventsDeatil->save();
							}
						}
					}
				}
				...
				$eventdetailMapper->getDbTable()->getAdapter()->commit();
				$response = array (
							"success" => true
					);
			} catch (Exception $ex) {
				...
				try {
					$eventdetailMapper->getDbTable()->getAdapter()->rollBack();
				} catch (Exception $e) {
	...
	public function gridAction() {
		...
		$time = $request->getParam("search_time");
		$request->setParam("search_time","");
		...
		$where = "";
		if($time==1) {
			$where = " AND ed.end_date_time<NOW() ";
		} else if($time==2)  {
			$where = " AND (NOW() <= ed.end_date_time) ";
		}
		...
		$select = $mapper->getDbTable ()->
					...
					from ( array ("e" => "module_events"),
							array (
									...
									"e.status" => "status",
									"e.order" => "order",
									"e.module_events_types_id"=>"module_events_types_id",
									"e.module_events_category_id"=>"module_events_category_id"))->
					joinLeft ( array ("ecd" => "module_events_category_detail"),
							"ecd.module_events_category_id = e.module_events_category_id AND ecd.language_id = ".$active_lang_id,
							array ("ecd.module_events_category_detail_id"=>"module_events_category_detail_id",
									"ecd.title" => "title",))->
					joinLeft ( array ("et" => "module_events_types_detail"),
							"et.module_events_types_id = e.module_events_types_id AND et.language_id = ".$active_lang_id,
							array ("et.module_events_types_detail_id"=>"module_events_types_detail_id",
									"et.title" => "title",))->
					joinLeft ( array ("ed" => "module_events_detail"),
							"ed.module_events_id = e.module_events_id AND ed.language_id = ".$active_lang_id,
							array (
					...
					where("e.customer_id=".$customer_id.$where)->order("e.order");
		...
		$select = $mapper->getDbTable ()->
			...
			foreach ( $rows as $rowId => $row ) {
				$edit = array();
				if($row [8] ["et.module_events_types_detail_id"]=="" && $row [8] ["e.module_events_types_id"]!="") {
					$mapper = new Events_Model_Mapper_ModuleEventsTypesDetail();
					$details = $mapper->fetchAll("module_events_types_id=".$row [8] ["e.module_events_types_id"]." AND language_id=".$default_lang_id);
					if(is_array($details)) {
						$details = $details[0];
						$row [8] ["et.title"] = $row[2] = $details->getTitle();
					}
				}
				if($row [8] ["ecd.module_events_category_detail_id"]=="" && $row [8] ["e.module_events_category_id"]!="") {
					$mapper = new Events_Model_Mapper_ModuleEventsCategoryDetail();
					$details = $mapper->fetchAll("module_events_category_id=".$row [8] ["e.module_events_category_id"]." AND language_id=".$default_lang_id);
					if(is_array($details)) {
						$details = $details[0];
						$row [8] ["ecd.title"] = $row[3] = $details->getTitle();
					}
				}
				if($row [8] ["ed.module_events_detail_id"]=="") {
					$mapper = new Events_Model_Mapper_ModuleEventsDetail();
					$details = $mapper->fetchAll("module_events_id=".$row [8] ["e.module_events_id"]." AND language_id=".$default_lang_id);
					if(is_array($details)) {
						$details = $details[0];
						$row [8] ["ed.title"] = $row[1] = $details->getTitle();
						$row [8] ["ed.start_date_time"] = $row[4] = $details->getStartDateTime();
						$row [8] ["ed.end_date_time"] = $row[5] = $details->getEndDateTime();
					}
				}
				if($row [8] ["ed.start_date_time"] != ""){
					$dateTime = new DateTime($row [8] ["ed.start_date_time"]);
					$row [8] ["ed.start_date_time"] = $row[4] = $dateTime->format("d/m/Y H:i");
				}
				if($row [8] ["ed.end_date_time"] != ""){
					$dateTime = new DateTime($row [8] ["ed.end_date_time"]);
					$row [8] ["ed.end_date_time"] = $row[5] = $dateTime->format("d/m/Y H:i");
				}
				...
				$response ['aaData'] [$rowId] [0] = '<input type="checkbox" name="check_all" class="check-box" value="'.$row [8] ["e.module_events_id"].'" />';
