			...
			if($form->isValid($this->_request->getParams())){
				try{
					...
					if ($request->getParam ( "module_website_id", "" ) == "") {
						...
						if($allFormValues['module_website_detail_id'] != null){
						    $currentWebsiteDetails = $websiteDetailMapper->getDbTable()->fetchAll("module_website_detail_id =".$allFormValues['module_website_detail_id'])->toArray();
						}else{
						...
						unset($allFormValues['module_website_detail_id'],$allFormValues['language_id']);
						if(count($websiteDetails) == count($customerLanguageModel)){
						    foreach ($websiteDetails as $websiteDetail) {
						        ...
						        $websiteDetailModel = new Website_Model_ModuleWebsiteDetail($websiteDetail);
						        $websiteDetailModel = $websiteDetailModel->save();
						    }    
						}else{
						    $websiteDetailMapper = new Website_Model_Mapper_ModuleWebsiteDetail();
						    $websiteDetails = $websiteDetailMapper->fetchAll("module_website_id =".$allFormValues['module_website_id']);
						    foreach ($websiteDetails as $websiteDetail){
